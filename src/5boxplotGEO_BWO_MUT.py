    ...
    for j in range(100):  
        ...
        for z in range(Cant_pruebas):
            ...
            for CurrentIteration in range(MaxIterations):
                ...
                # 3.----MUTACIÓN
                Pop3, Fit_mutation = bwo.mutacion(Pop1, PM,Celdas_LB, Celdas_UB, Celdas_canti, Rutas_piezas,M_secuencias, M_tiempos, Cost_rutas,Vol_piezas,MTBF_maq, Cost_break, limites)
                #_________________________________________________________________________________________________________________________________________________________________------
                # 4.----ACTUALIZACIÓN DE LA POBLACIÓN
                Pop = np.vstack((Pop, Pop3))

                Fit_pop = np.concatenate((M_fitness, Fit_mutation))
        ...
        print(rpd) 
        if rpd < 1.5:
            np.savez("results_GEO-BWO-PM.npz", fitness_histories=fitness_histories)
            ...
            # Guardar como Excel
            with pd.ExcelWriter("results_GEO-BWO-PM.xlsx", engine="xlsxwriter") as writer:
            ...
            grafi = gr.graficos(fitness_histories, MaxIterations,"GEO_BWO_PM")
            return x_best, fval
